...
if(isset($_POST['submit'])){
    ...
    if(empty($userEmail)){
      header("Location: usersFitnessAdded.php?error = email is required");
      exit();
    }else if(empty($name)){
      header("Location: usersFitnessAdded.php?error = Name is required");
      exit();
    }else{
      $fitness = $MRFitness->fitness;
      $result = $fitness->insertOne(['userEmail' => "$userEmail", 'fitnessstatus' => "$name"]);
    if($result){
      var_dump($result);
      header("Location: usersfitness.php");
    }else{
      header("Location: usersfitness.php?error = Duplicate data");
    }
}
}
?>
