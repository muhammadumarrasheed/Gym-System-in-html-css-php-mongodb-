...
if(isset($_POST['submit'])){
  ...
  if(empty($name)){
    header("Location: userDataIn.php?error = Name is required");
    exit();
    ...
    header("Location: userDataIn.php?error = Phone number is required");
    exit();
  }else if(empty($city)){
  ...
  if($MRFitness->user->InsertOne(['userEmail' => $email, 'username' => $name, 'password' => $password, 'phoneNum' => $phone, 
    ...
    $planfit = "None";
    $MRFitness->plan->InsertOne(['userEmail' => $email, 'palanname' => $planfit]);
    $MRFitness->fitness->InsertOne(['userEmail' => $email, 'fitnessstatus' => $planfit]);

    header("Location: userData.php");
  }else{
    header("Location: userData.php?error = Already user with similar email sign uped");
  }
}
}
?>
