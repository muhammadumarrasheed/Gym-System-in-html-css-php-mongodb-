...
if(isset($_POST['submit'])){
    ...
    $name = validate($_POST['status']);
  
    if(empty($userEmail)){
      header("Location: userPlanAdded.php?error = email is required");
      exit();
    }else if(empty($name)){
      header("Location: userPlanAdded.php?error = Name is required");
      exit();
    }else{
      $plan = $MRFitness->plan;
      $result = $plan->insertOne(['userEmail' => "$userEmail", 'planname' => "$name"]);
    if($result){
      var_dump($result);
      header("Location: usersPlans.php");
    }else{
      header("Location: usersPlans.php?error = Primary key voilation");
    }
}
}
?>
    ...
    <div class="container my-5">
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" placeholder="Enter diet plan" name="status" autocomplete="off">
            </div>
