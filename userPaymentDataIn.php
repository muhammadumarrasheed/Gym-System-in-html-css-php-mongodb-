...
if(isset($_POST['submit'])){
  ...
  $cmd = $MRFitness->user->find();
      foreach($cmd as $row){
          if($row['username'] == $name){
              $userEmail = $row['userEmail'];
          }
      }
      $cd = $MRFitness->userpayment->find();
      foreach($cd as $r){
          if($r['userEmail'] = $userEmail){
            header("Location: userPaymentData.php?error = Payment data is already stored");
          }
      }
    $result = $MRFitness->userpayment->insertOne(['userEmail' => "$userEmail", 'paymentStatus' => "$status",'paymentDate'=>$date]);
    if($result){
    ...
    <div class="container my-5">
        ...
        <form method="post">
            <div class="form-group">
                <label class="title">Add user payment</label>
            </div>
            ...
            <div class="form-group">
                ...
                <input type="date" class="form-control" placeholder="dd---yyyy" name="date" autocomplete="off">
            </div>
