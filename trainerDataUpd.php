...
$var = $_GET['updateid'];
$command = $MRFitness->trainer->find(array('trainerEmail' => $var));
...
if($MRFitness->trainer->updateOne(
    ['trainerEmail' => $var], 
    ['$set'=> ['trainerEmail' => $var, 'trainername' => $name, 'password' => $password, 'phoneNum' => $phone, 
    'city' => $city, 'postacode' => $postal,'streetID' => $street, 'houseNum' => $house]]
)){
    header("Location: trainerData.php");
}else{
header("Location:  trainerData.php?error = Data is duplicated");
}
}?>
<!doctype html>
<html lang="en">
