...
if(isset($_POST['submit'])){
  ...
  $trainername= validate($_POST['trainername']);
  $status = validate($_POST['status']);
  $date = validate($_POST['date']);
$cmd = $MRFitness->trainerpayment->find();
    foreach($cmd as $row){
        if($row['trainername'] == $trainername){
            $trainerEmail = $row['trainerEmail'];
        }
    }
       $result = $MRFitness->trainerpayment->insertOne(['trainerEmail' => "$trainerEmail", 'paymentStatus' => "$status",'paymentDate'=>$date]);
  if($result){
    header("Location: trainerPaymentData.php");
  }else{
    header("Location: trainerPaymentData.php?error = Primary key voilation");
  }
}
?>
<?php
